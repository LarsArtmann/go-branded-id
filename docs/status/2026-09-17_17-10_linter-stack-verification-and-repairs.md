...
|---|------|-----------|--------------|--------|
| 1 | go-finding flake `test` app still root-only | Gap is documented truthfully in AGENTS.md | Flake app itself not fixed to iterate all 5 modules (go-output-style loop) | S–M |
| 2 | cmdguard cleanup-hook context design | Lint silences the finding; behavior verified unchanged | No design review of whether hooks *should* capture the Execute-time ctx explicitly instead of nolint | S |
| 3 | cmdguard fix documentation | nolint comment in code with reason | AGENTS.md entry (cobra RunE deferred-context footgun) not written | S |
| 4 | linter-building skill refresh | go-finding row + 2 new ecosystem rows updated | Decision-shortcuts section not extended (need-CLI-shell→cmdguard etc.); ~10 other verification rows still dated 2026-09-09 | M |
| 5 | go-linter-sdk release hygiene | Fix is committed and tests green | CHANGELOG.md / TODO_LIST.md not updated for the flake fix (daemon commit `1a7d7c8` carries a heuristic message; the CHANGELOG-narrative rule from its own AGENTS.md is unmet) | S |
| 6 | Cross-repo version alignment | Current pins verified and recorded | go-linter-sdk still requires go-finding v1.10.0 while v1.12.0 is published — deliberate bump not executed | S |
...
|---|------|-----------------|----------|
| 1 | Rebuild cmd/namer as a real linter (go-linter-sdk rules emitting `finding.Finding`, cmdguard CLI shell, go-output rendering) | Session's implied goal; waiting on repo-placement + suppression-design decisions (see g) | HIGH |
| 2 | go-linter-sdk first-production-consumer milestone (v0.4.0) | Blocked by 1 | HIGH |
| 3 | `nix flake check` pass over all four repos | Not run this session (tests only) | MEDIUM |
| 4 | Race verification for go-linter-sdk + go-output | Not run this session | MEDIUM |
| 5 | docs-health HARVEST of section (f) into TODO_LIST.md / ROADMAP.md | Report was just written; HARVEST is the declared next step of the status-report loop | MEDIUM |
| 6 | go-branded-id AGENTS.md consumer-list update (+go-output, +cmdguard, +go-finding CLI) | Discovered late in session; memory-maintenance miss | HIGH (cheap) |
| 7 | Trust-engineering groundwork for the brand-name linter (golden corpus, positive/negative fixtures, discrimination proof, 14-repo sweep) | Blocked by 1 | HIGH after 1 |
| 8 | Fleet "stack doctor": one command checking version alignment across the 5-repo dep graph | Idea from this session (would have caught go-linter-sdk@v1.10.0 vs v1.12.0 lag) | MEDIUM |
| 9 | go-finding release-procedure.md out-of-sync annotation (docs-health ANNOTATE) | Pre-existing warning, unrelated to session | LOW |
| 10 | Website/docs sync for any repo | Nothing user-facing changed | LOW |
...
4. **Concurrent session/release interleaving.** v1.12.0 was cut *while* this session was verifying; release commits, daemon heuristic commits, and my fixes interleave (my README fix was absorbed into `72061b0`, a "4 changed files" heuristic commit). Severity: attribution and bisectability of history suffer; no data lost.

**My own misses (radical honesty):**

5. **Broken-filter misread.** `grep -v "/"` on go.mod output filtered out literally every line → I briefly suspected the daemon had regressed sub-module pins when the lines had simply moved to v1.12.0 via the legit release commit. Caught on re-verification, no damage, but I violated the pipeline-masking rule *during a session about verification rigor*.
...
|---|------|--------|--------|----------|
| 1 | Decide cmd/namer endgame placement (in-repo rebuild vs standalone linter repo; golangci plugin or not) — blocks the whole chain | Critical | S | Decision |
| 2 | Rebuild cmd/namer on go-linter-sdk: one RuleFunc per rule emitting finding.Finding, Registry, ExitCodeByConfidence | Critical | L | Feature |
| 3 | Add in-source suppression support for deliberate non-brands (go-cqrs-lite markers): `//branded-id:ignore(<rule>) <reason>` design | High | M | Feature |
| 4 | Root-cause the daemon clobber (how 802ff0a got a stale flake.nix); add a guard or document the mechanism | High | M | Bug |
| 5 | Fix go-finding flake `test` app to iterate all 5 modules (port go-output's loop) | High | S | Bug |
| 6 | Add dead-gate assertion: test app must cover every go.work `use` entry (fails loudly if a module is skipped) | High | S | Quality |
| 7 | Golden corpus for the brand-name linter: positive + negative fixtures per rule, measured FP rate on the 14 downstream repos | High | L | Quality |
| 8 | Discrimination proof for the rebuilt linter (mutant analyzer must fail the corpus) | High | S | Quality |
| 9 | go-branded-id AGENTS.md: add go-output, cmdguard, go-finding CLI to the consumer list | High | S | Documentation |
| 10 | HARVEST this report's (f) into the right repos' TODO_LIST.md / ROADMAP.md (docs-health) | High | M | Documentation |
| 11 | go-linter-sdk: CHANGELOG + TODO_LIST entries for the flake `self,` regression story | Medium | S | Documentation |
| 12 | Extend docs-api-check.sh to validate every README version stamp against tags/version.go (or single-source the table) | High | M | Quality |
| 13 | Bump go-linter-sdk to go-finding v1.12.0 (go-ecosystem-upgrade flow; currently 2 minors behind) | Medium | S | Cleanup |
| 14 | `nix flake check` across all four repos; record results | Medium | S | Quality |
| 15 | Race verification for go-linter-sdk + go-output | Medium | S | Quality |
| 16 | cmdguard AGENTS.md: document the cobra deferred-context pattern + nolint rationale (COBRA_FOOTGUNS.md candidate) | Medium | S | Documentation |
| 17 | cmdguard cleanup-hook design review: explicit ctx capture vs accepted nolint | Medium | S | Quality |
| 18 | Write the golden-path doc: "shipping a LarsArtmann linter in 2026" (go-linter-sdk + go-finding + cmdguard + go-output wiring, one page) | High | M | Documentation |
| 19 | Fleet stack-doctor script: verify go-branded-id/go-output/go-finding/cmdguard/go-linter-sdk version alignment in one command | Medium | M | Quality |
| 20 | linter-building skill: extend decision-shortcuts with cmdguard (CLI shell) and go-output (presentation) rows | Medium | S | Documentation |
| 21 | linter-building skill: re-verify remaining 2026-09-09 verification rows against current sources | Medium | M | Documentation |
| 22 | Extract fleet template: "flake.nix module-loop test app" (reference: go-output) for all multi-module repos | Medium | M | Quality |
| 23 | go-output AGENTS.md: fix stale "currently v0.37.0" pin text → v0.38.0 | Medium | S | Documentation |
| 24 | cmd/namer: keep codemod mode (`-write` stub generation) as FixStrategyDirect findings instead of a parallel flag path — design note | High | S | Feature |
| 25 | golangci-lint v2 module-plugin packaging decision for the brand linter (go-humanize-linter pattern) | Medium | M | Decision |
| 26 | toolsdk Spec for the brand linter (BuildFlow provider integration, DetectorFromRegistry) | Medium | M | Feature |
| 27 | SARIF output + GitHub code-scanning upload decision for the brand linter | Medium | S | Decision |
| 28 | Baseline/ratchet support decision for rolling the brand linter into 14 downstream repos | Medium | S | Decision |
| 29 | Suppression staleness policy for brand suppressions (expiry? review date?) | Low | S | Decision |
| 30 | Verify pkg.go.dev renders go-finding v1.12.0 + refresh "Imported by" counts (post-release habit) | Low | S | Cleanup |
| 31 | Verify go-linter-sdk v0.3.1 pkg.go.dev README rendering (frozen-README habit) | Low | S | Cleanup |
| 32 | go-linter-sdk benchmarks: run once, store baseline | Low | S | Quality |
| 33 | go-linter-sdk examples: verify all 4 run under both GOEXPERIMENT modes | Low | S | Quality |
| 34 | go-finding: annotate release-procedure.md out-of-sync warning (docs-health ANNOTATE) | Low | S | Documentation |
| 35 | go-finding: verify v1.12.0 GitHub Release notes came from the CHANGELOG section (release-workflow contract) | Low | S | Quality |
| 36 | Investigate whether BuildFlow covers go-finding/go-output/cmdguard (.buildflow.yml presence); align or document why not | Low | S | Cleanup |
| 37 | Fleet doc: single landing section linking the four stack repos and their roles | Low | S | Documentation |
| 38 | go-linter-sdk: consider `WithToolName` auto-stamp coverage test for future consumers | Low | S | Quality |
| 39 | cmdguard: coverage 87.8% → push the remaining core-package gap (their TODO list governs) | Low | M | Quality |
| 40 | Cross-repo CI: flake-check job in go-linter-sdk CI would have caught the `self,` regression in ~30s — verify it exists, add if not | High | S | Bug |
| 41 | go-branded-id: confirm v0.5.1 is the latest tag; release if newer consumers-facing changes are pending | Low | S | Cleanup |
| 42 | Add "capture repo baselines before multi-repo verification" to the global workflow habits (AGENTS.md cross-cutting lesson) | Medium | S | Documentation |
| 43 | Adopt an explicit rule: "a fix for a found regression gets a named commit when policy allows" (see g3) | Medium | S | Documentation |
| 44 | go-finding: add a README module-table freshness note to the release checklist (stop shipping stale stamps) | Medium | S | Documentation |
| 45 | Sweep the 14 downstream repos for brand types missing Name() using the rebuilt linter; record FP rate | High | L | Quality |
| 46 | Decide go-cqrs-lite marker-brand story end-to-end: suppression examples in the linter docs | Medium | S | Documentation |
| 47 | go-output: verify no root-module drift (core invariant) after v0.38.0 — quick grep gate | Low | S | Quality |
| 48 | cmdguard: re-run full check-all after any future golangci-lint upgrade (contextcheck behavior drift risk) | Low | S | Quality |
| 49 | Skill feedback loop: pipeline-masking violation happened in-session — strengthen the lesson with this concrete example in global AGENTS.md | Medium | S | Documentation |
| 50 | Schedule a brutal-self-review pass over this session's repairs (flake fix, nolint) in ~2 weeks to confirm nothing regressed | Low | S | Quality |
...
*Point-in-time snapshot. States re-verify against `git log` before relying on them (repos had concurrent activity during this session — go-finding released v1.12.0 mid-session).*
