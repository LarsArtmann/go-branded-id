...
- **pnpm 11 reads `overrides` from `pnpm-workspace.yaml`, NOT `package.json`.** A top-level `overrides` block in `package.json` is npm syntax and is silently ignored by pnpm (and `pnpm.overrides` is deprecated in v11 — install warns "no longer read"). Security version pins live in `pnpm-workspace.yaml` under `overrides:`. Also: `pnpm update <pkg>` only touches direct dependencies — transitive deps need an override.
