...
| #   | Item                       | Status  | Notes                    |
| --- | -------------------------- | ------- | ------------------------ |
| A1  | Tests pass with `-race`    | ✅ DONE | 89 tests, 0 failures     |
| A2  | Coverage adequate          | ✅ DONE | 81.4% statement coverage |
| A3  | Lint clean (golangci-lint) | ✅ DONE | 0 issues                 |
| A4  | Build succeeds             | ✅ DONE | `go build ./...` clean   |
| A5  | Benchmarks run             | ✅ DONE | All 26 benchmarks pass   |
| A6  | v0.3.0 tag exists          | ✅ DONE | Tag at `d7015ce`         |
| A7  | git status clean           | ✅ DONE | No uncommitted changes   |

---

## Category B: Critical Release Blockers

| #   | Item                         | Status     | Notes                                                                                                                                                |
| --- | ---------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | **Fix v0.3.0 tag position**  | ⏳ BLOCKED | Tag points to `d7015ce`, 5 commits behind HEAD `4f76c3c`. Missing: `MustValidateID`, benchmarks, fuzz tests. **NEEDS USER APPROVAL for force-push.** |
| B2  | Re-trigger Release CI        | ⏳ BLOCKED | gh CLI auth expired. Cannot trigger workflows without re-auth.                                                                                       |
| B3  | Verify GitHub Release exists | ⏳ BLOCKED | Release CI failed on old commit. Cannot verify without gh auth.                                                                                      |
| B4  | Fix `gh` CLI auth            | 🔜 TODO    | User needs to run `gh auth login`. **NOT MY JOB — user action required.**                                                                            |
...
| --- | -------------- | ---------------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------- |
| D1  | storbi         | `:=` instead of `=` in `internal/di/container.go:41,46,51,56,61,66,71` | 🔄 IN PROGRESS | 7 occurrences of `err :=` where `err` already declared. Fix: change to `err =`.                   |
...
| E2  | Audit for missed `.String()` calls        | 🔜 TODO | Search all repos for `.String()` usages that should be `.Get()` |
...
| #   | Item                                    | Status         | Notes                                                     |
| --- | --------------------------------------- | -------------- | --------------------------------------------------------- |
| F1  | Create codemod tool                     | 🔜 TODO        | Auto-generate `Name()` stubs for brand types              |
...
| F4  | Verify storbi pre-existing build errors | 🔄 IN PROGRESS | See D1 above                                              |
...
_Generated by Crush on 2026-05-22_
